...
(define (eq-pair-in-list pair lst)
  ...
  (map (λ (x) (if (and (= (car pair) (car x)) (= (cdr pair) (cdr x))) 1 0) (car lst)) lst))

(define (size lst max_x min_x max_y min_y type x y)
  (if (empty? lst) (list (- max_x min_x) (abs min_x) (- max_y min_y) (abs min_y))
      (if (integer? (car lst))
          (cond ((= type 0) (size (cdr lst) (max max_x (+ x (car lst))) min_x max_y min_y type (+ x (car lst)) y))
                ((= type 1) (size (cdr lst) max_x min_x (max max_y (+ y (car lst))) min_y type x (+ y (car lst))))
                ((= type 0) (size (cdr lst) (max max_x (+ x (car lst))) min_x max_y min_y type (+ x (car lst)) y))
                ((= type 1) (size (cdr lst) max_x min_x (max max_y (+ y (car lst))) min_y type x (+ y (car lst)))))
          (size (cdr lst) max_x min_x max_y min_y (if (equal? (car lst) #\L) (remainder (+ type 1) 4) (if (= type 0) 3 (remainder (+ type 1) 4))) x y))))

(define (vect vec lst)
  (for-each (λ (x) (vector-set! vec (car x) (+ (cdr x) (vector-ref vec (car x))))) lst) vec)
;доделать
;тот же робот, только нужно найти кратчайший путь от того места, куда он пришел, но двигаться можно только по тем клеткам, которые мы уже прошли
; система команд: П Л Вверх Вниз
