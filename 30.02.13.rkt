...
(define (writeln lst n)
  (define (help lst lst1 lst2 lst3 o)
    (if (member (car lst) lst3) (string (map (λ (x) (smile x) (list (reverse (cons #\. lst1)) (reverse (cons #\( lst2)) (reverse (cons #\) lst3))))))
        (if (< o n) (help (cdr lst) (cons (car lst) lst1) lst2 lst3 (+ o 1))
            (if (= o n) (help (cdr lst) (cons (car lst) lst1) lst2 lst3 (+ o 1))
                (if (member (car lst) lst2) (help (cdr lst) lst1 lst2 (cons (car lst) lst3) o)
                    (help (cdr lst) lst1 (cons (car lst) lst2) lst3 o))))))
  (help lst '() '() '() 0))
